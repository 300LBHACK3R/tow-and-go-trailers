...
export function TrailerCard({ trailer }: TrailerCardProps) {
  return (
    <article
      id={trailer.id}
      className="group overflow-hidden rounded-[2rem] border border-white/10 bg-black/40 shadow-[0_20px_60px_rgba(0,0,0,0.45)] backdrop-blur-sm transition duration-300 hover:-translate-y-1 hover:border-[#d4af37]/30"
    >
      ...
      <div className="p-6">
        <div className="flex flex-wrap gap-2">
          {trailer.specs.slice(0, 3).map((item) => (
            <span
              key={item}
        ...
        <h4 className="mt-6 text-sm font-semibold uppercase tracking-[0.22em] text-[#d4af37]">
          Key Specs
        </h4>
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2">
          <Button href={`/contact?trailer=${encodeURIComponent(trailer.name)}`} className="w-full">
